...
int GetEXCTMUX(int *x)       { return(x[EXCTMUX]); }
int GetCHECKMUX(int *x)      { return(x[CHECKMUX]); }
...
void load_program(char *program_filename) {
    ...
    /*if (strcmp(program_filename, "add")==0) CURRENT_LATCHES.PC = (program_base << 1);*/
...
int OperateJ(int x){
    ...
    if (GetCOND(CURRENT_LATCHES.MICROINSTRUCTION)==4) {
        /*printf("PSR: 0x%0.4x\n",CURRENT_LATCHES.PSR);*/
        if (((CURRENT_LATCHES.PSR&0x8000)>>15)==1)
        {
            /*printf("right\n");*/
            return (x+8);
        }
...
void eval_micro_sequencer() {
    ...
    {
        if(GetCOND(CURRENT_LATCHES.MICROINSTRUCTION)==6)
        {
        ...
        else if(GetCOND(CURRENT_LATCHES.MICROINSTRUCTION)==7)
        {
            ...
            NEXT_LATCHES.STATE_NUMBER=OperateJ(GetJ(CURRENT_LATCHES.MICROINSTRUCTION));
...
void latch_datapath_values() {
    ...
    if (GetLD_EXCT(CURRENT_LATCHES.MICROINSTRUCTION)) {
        ...
        if (GetEXCTMUX(CURRENT_LATCHES.MICROINSTRUCTION)==0) {
            if ((BUS<0x3000)&&((CURRENT_LATCHES.PSR&0x8000)>>15)) {
                NEXT_LATCHES.EXCT=1;
                /*printf("right\n");*/
            }
            else {

...
void MEM_stage() {
    ...
    if (v_dcache_en)
    {
        ...
        if (Get_DCACHE_RW(PS.MEM_CS))
        {
            ...
            else
            {
                if (PS.MEM_ADDRESS&0x0001)
                {
                    mem_w1=1;
                    mem_w0=0;
                    write_word=((PS.MEM_ALU_RESULT&0x00FF)<<8);
                }
                else
                {
                    mem_w1=0;
                    mem_w0=1;
                    write_word=(PS.MEM_ALU_RESULT&0x00FF);
                }
            }
        }
        ...
        dcache_access(PS.MEM_ADDRESS, read_word, write_word, dcache_r,
                      mem_w0, mem_w1);
        
        mem_stall=(~mem_r)&v_dcache_en;
        
        
    }
    else mem_stall=0;
    if (Get_DATA_SIZE(PS.MEM_CS)) {
        NEW_PS.SR_DATA=readword;
    }
    else{
        if (PS.MEM_ADDRESS&0x0001) {
            readword=(readword&0xFF00)>>8;
            NEW_PS.SR_DATA=sext(readword, 8);
        }
        else{
    ...
    if (PS.MEM_V) {
        ...
        if (Get_BR_OP(PS.MEM_CS)) {
            if (((PS.MEM_IR&0x0E00)>>9)&PS.MEM_CC) {
                mem_pcmux=1;
...
int GetSHFvalue(){
    int a= PS.AGEX_IR&0x000F;
    int b = 0xFFFF<<(16-a);
    if ((PS.AGEX_IR&0x0010)==0) {
        printf("outcome: %d\n",Low16bits(PS.AGEX_SR1<<a));
        return (Low16bits(PS.AGEX_SR1<<a));
        
    }
    if ((PS.AGEX_IR&0x0020)==0){
        return (Low16bits(PS.AGEX_SR1>>a));
        
    }
    return (Low16bits((PS.AGEX_SR1>>a)|b));
...
void AGEX_stage() {
    ...
    if (LD_MEM) {
        ...
        if (Get_ADDR1MUX(PS.AGEX_CS)){
            addr1value=PS.AGEX_SR1;
            
        }
        else addr1value=PS.AGEX_NPC;
        
        if (Get_ADDR2MUX(PS.AGEX_CS)==0) {
            addr2value=0;
        }
        else if (Get_ADDR2MUX(PS.AGEX_CS)==1) {
            addr2value=sext(PS.AGEX_IR, 5);
        }
        else if (Get_ADDR2MUX(PS.AGEX_CS)==2) {
            addr2value=sext(PS.AGEX_IR, 8);
        }
        else {
            addr2value=sext(PS.AGEX_IR, 10);
        }
        
        if (Get_LSHF1(PS.AGEX_CS)) {
            addr2value=(addr2value<<1);
        }
        
        if (Get_ADDRESSMUX(PS.AGEX_CS)) {
            addressvalue=Low16bits(addr1value+addr2value) ;
        }
        else{
            addressvalue=((PS.AGEX_IR&0x00FF)<<1);
        }
        
        if (Get_SR2MUX(PS.AGEX_CS)) {
            sr2value=sext(PS.AGEX_IR, 4);
        }
        else sr2value=PS.AGEX_SR2;
        
        if (Get_ALU_RESULTMUX(PS.AGEX_CS)) {
            if (Get_ALUK(PS.AGEX_CS)==0) {
                alureslutvalue=sr2value+PS.AGEX_SR1;
            }
            ...
            else alureslutvalue=sr2value;
        }
        else {
            alureslutvalue=GetSHFvalue();
        }
    
        alureslutvalue=Low16bits(alureslutvalue);
        
        NEW_PS.MEM_ADDRESS=addressvalue;
        NEW_PS.MEM_NPC=PS.AGEX_NPC;
        NEW_PS.MEM_CC=PS.AGEX_CC;
        NEW_PS.MEM_ALU_RESULT=alureslutvalue;
...
void DE_stage() {
    ...
    CONTROL_STORE_ADDRESS=((PS.DE_IR&0x0020)>>5)+((PS.DE_IR&0xf800)>>10);
    
    de_sr1=(PS.DE_IR&0X01C0)>>6;
    if ((PS.DE_IR&0x0200)>>13) {
        de_sr2=(PS.DE_IR&0x0E00)>>9;
    }
    else de_sr2=PS.DE_IR&0x0007;
    printf("de_sr2: %d\n",de_sr2);
    printf("agex_drid: %d\n",PS.AGEX_DRID);
    ...
    if (Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS])) {
        if ((PS.AGEX_DRID==de_sr1)&v_agex_ld_reg) {
            printf("yeah!");
            dep_stall=PS.DE_V;
        }
        else if ((PS.MEM_DRID==de_sr1)&v_mem_ld_reg) {
            dep_stall=PS.DE_V;
        }
        else if ((PS.SR_DRID==de_sr1)&v_sr_ld_reg) {
            dep_stall=PS.DE_V;
        }
        else dep_stall=0;
    }
    else if (Get_SR2_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS])) {
        if ((PS.AGEX_DRID==de_sr2)&v_agex_ld_reg) {
            dep_stall=PS.DE_V;
        }
        else if ((PS.MEM_DRID==de_sr2)&v_mem_ld_reg) {
            dep_stall=PS.DE_V;
        }
        else if ((PS.SR_DRID==de_sr2)&v_sr_ld_reg) {
            dep_stall=PS.DE_V;
        }
        else dep_stall=0;
    }
        ...
        if (v_agex_ld_cc|v_mem_ld_cc|v_sr_ld_cc) {
            printf("v_agex_ld_cc: %d \n",v_agex_ld_cc);
            printf("v_mem_ld_cc: %d \n",v_mem_ld_cc);
            printf("v_sr_ld_cc: %d \n",v_sr_ld_cc);
            dep_stall=PS.DE_V;
        }
        else dep_stall=0;
    ...
    if (LD_AGEX) {
        ...
        else NEW_PS.AGEX_DRID=(PS.DE_IR&0x0E00)>>9;
...
void FETCH_stage() {
    ...
    /* how to generate these two signals?*/
    
    ir_r=&icache_r;
    read_word=&de_ir;
    icache_access(PC,read_word,ir_r);
    printf("icache_r is %d\n",icache_r);
    
    /*generate ld.de*/
    
    if (mem_stall==1||dep_stall==1) {
        LD_DE=0;
    }
    else LD_DE=1;
    ...
    if (v_mem_br_stall==1&&mem_stall==0)
        LD_PC=1;
    else if (dep_stall==0&&v_de_br_stall==0&&v_agex_br_stall==0) {
        LD_PC=icache_r;
        printf("LD_PC's value is %d\n",LD_PC);
    }
    else LD_PC=0;
